...
public class BenchmarkRunner {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(csvFile))) {
            pw.println("GridSize,SearchFactor,SerialTime,ParallelTime,Speedup");
            ...
            for (int grid : gridSizes) {
                for (double factor : searchFactors) {
                    ...
                    pw.printf("%d,%.2f,%d,%d,%.2f%n",
                            grid, factor, avgSerialTime, avgParallelTime, speedup);

                    System.out.printf("Grid %d, Factor %.2f → Serial %d ms (avg), Parallel %d ms (avg), Speedup %.2f%n",
                            grid, factor, avgSerialTime, avgParallelTime, speedup);
                }
            }
        }
